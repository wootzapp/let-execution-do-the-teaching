...
*How far can we push a coding model when execution—not humans—does the grading?*

September 2026

> **Abstract.** Improving a coding model is a sequence of decisions: find
> the limiting behavior, create useful practice, and determine which
> attempts deserve reinforcement. We investigate a practical way to make
> those decisions increasingly repeatable by grounding feedback in
> execution. Starting from GLM-4.7-Flash, the work combines whole-file
> supervised fine-tuning, targeted C++ repair curricula, group-relative
> reinforcement learning, and verifiers that turn program behavior into
> structured rewards. A Phone Number task makes the method concrete: a
> compiler failure or an invalid accepted input becomes a specific repair
> context, and executable checks grade sampled solutions. The released
> evaluations span supervised and reinforcement-learning checkpoints. The
> highest reported GLM row reaches 45.2% first-round success and 61.5%
> within two test rounds on Fixed26, a 26-task assisted development suite;
> this Generalized C++ row uses four selected trials and includes six
> training-task overlaps. The contribution is a connected recipe for
> moving repeated judgment into reusable task, reward, and runtime
> machinery. [[1, 2, 3, 4, 5, 6]](#references)

# 1 Introduction: make the next improvement actionable
...
## A learning system around the model
...
# 2 Related work: three questions behind the method
...
programs. [[8]](#references) It establishes execution as a source of informative
supervision. DeepSeekMath introduces Group Relative Policy Optimization
(GRPO), which compares rewards among samples for the same problem. [[7]](#references)
...
tests in real codebases. [[9]](#references) FrogNano's TaskPilot adapts task
generation and refinement to the current policy's observed success rate.
[[11]](#references) These approaches make task construction part of the learning
...
agent-computer interface shapes software-engineering performance. [[10]](#references)
...
matrices adapt the model while its base weights remain fixed. [[12]](#references) The
project's GLM/Miles integration carries those adapters through training,
rollout serving, and evaluation.

## From these ingredients to a usable recipe
...
# 3 Methodology: From a failure to a lesson
...
flowchart LR
    D["`**1 DIAGNOSE**
    Inspect an executed failure`"]
    P["`**2 DESIGN PRACTICE**
    Choose a targeted exercise`"]
    X["`**3 EXECUTE & SCORE**
    Judge sampled solutions`"]
    U["`**4 UPDATE**
...
**Figure 1.** The improvement workflow. Diagnosis and curriculum choices
...
Measured outcomes inform the next intervention.

## 3.1 Begin with a behavior, not just a score
...
the outcome concrete enough to verify. [[2]](#references)
...
complete coding task around it. [[2]](#references)
...
## Give the learner a useful signal

## 3.3 Use execution to check both the exercise and the answer
...
behavior being taught. [[13]](#references)
...
checks are grouped into four policy categories. [[2]](#references)
...
## 3.4 Choose tasks that offer room to learn
...
practice. [[4, 7]](#references)

## 3.5 Reinforce the more successful attempts
...
asks what the resulting policy can do. [[2, 5, 6]](#references)

# 4 Engineering: Make the feedback worth learning from

*Every engineering choice protects a link between model behavior and
learning.*

## 4.1 Turn a generated response into the intended program
...
## 4.2 Let an environment fault remain an environment fault
...
investigation. [[5]](#references)

## 4.3 Preserve the policy across training and serving
...
a much larger distributed system. [[5]](#references)

## 4.4 Grow a task-specific judge into shared infrastructure
...
incomplete candidates receive differentiated non-positive scores. [[4]](#references)
...
to both global correctness and required topic coverage. [[4]](#references)
...
feedback. A test round may contain multiple model requests. [[1, 6]](#references)
...
## 5.2 Read the release as a sequence of experiments
...
progression with these conditions attached. [[1]](#references)

## 5.3 The recorded training recipe
...
prompts with 32 responses each; learning rate is 3 × 10⁻⁵, with
KL coefficient 0.1. The scored adapter is iteration 14. Appendix A
gathers the full settings and the result-specific evidence checks. [[6]](#references)

## From useful edits to stronger completion
...
**Figure 2.** Mean tasks solved out of 26, from the release results. Four
...
set of configurations appears below. [[1]](#references)

| **Configuration** | **First round mean / 26** | **Two rounds mean / 26** | **Conditional recovery** |
| --- | --- | --- | --- |
| GLM-4.7-Flash base | 0.50 (1.9%) | 4.50 (17.3%) | 16/102 (15.7%) |
| Luna reference | 6.25 (24.0%) | 16.75 (64.4%) | 42/79 (53.2%) |
| SFT v5 | 6.00 (23.1%) | 10.25 (39.4%) | 17/80 (21.2%) |
| Synth v1, epoch 50 | 9.50 (36.5%) | 12.00 (46.2%) | 10/66 (15.2%) |
| Execution-midband RL v1 | 8.25 (31.7%) | 13.00 (50.0%) | 19/71 (26.8%) |
| Execution-midband RL v2 | 10.50 (40.4%) | 14.50 (55.8%) | 16/62 (25.8%) |
| Phone Number, iter 14 | 11.25 (43.3%) | 15.25 (58.7%) | 16/59 (27.1%) |
| Generalized C++, iter 14\* | 11.75 (45.2%) | 16.00 (61.5%) | 17/57 (29.8%) |
...
training overlap described in Section 5.2. [[1]](#references)

## 5.4 What changes across the reported stages?
...
## A better average is the beginning

*The interesting next lesson often appears inside the result.*

## 5.5 Solving and repairing move differently
...
a useful reference point under its separate hosted setup. [[1]](#references)
...
## 5.6 Return to the Phone Number example
...
lineage. They make retention and broader effects concrete. [[6]](#references)

| **Task** | **Synth v1 first-round successes** | **Phone Number first-round successes** |
| --- | --- | --- |
| Phone Number | 1 / 4 | 4 / 4 |
| Parallel Letter Frequency | 0 / 4 | 3 / 4 |
| Gigasecond | 3 / 4 | 0 / 4 |
| Sublist | 3 / 4 | 1 / 4 |
...
## 5.7 Use uncertainty to choose the next experiment
...
# 6 Discussion: Put expertise where it compounds

## 6.1 What execution takes over
...
## 6.2 What a practitioner can take away
...
## 6.3 Where collaborators can contribute
...
# Appendix A: Recipe and reward contracts

## A.1 Phone Number training configuration

| **Setting** | **Recorded value** |
| --- | --- |
| Base model and adaptation | GLM-4.7-Flash; LoRA rank 16, alpha 32. |
| Training hardware | 8 × H100 80 GB; BF16; tensor parallel 4; expert parallel 8. |
| Dispatch and serving | DeepEP flex dispatch; SGLang rollout service; Miles training integration. |
| Rollout and batch | 8 prompts × 32 samples; global batch 256; 20 GRPO updates. |
| Optimization | Learning rate 3 × 10⁻⁵; constant schedule; warmup fraction 0.1; KL coefficient 0.1. |
...
describing the run. [[6]](#references)

## A.2 Reward contracts are experiment-specific

| **Reward path** | **Definition and role** |
| --- | --- |
| Shared Aider reward | Default path: prohibited edit/runtime -1; other parse failure -0.8; compile failure or candidate timeout -0.5; full pass +1; otherwise 0.6 × test fraction. Recoverable format penalty 0.1; clipped to [-1,1]. A strict binary mode is also implemented. |
...
| Current topic-aware composition | A positive global score is preserved when required topic checks pass. Otherwise use 0.5 × min(global score,0) + 0.5 × (topic requirement fraction -1). Invalid topic evaluation is flagged separately. |

Sources: shared reward implementation [[3]](#references), Phone Number [[2]](#references),
generalized and topic-aware rewards [[4]](#references). The current generalized
...
# Appendix B: Results, artifacts, and reproduction

## B.1 Complete dispersion for the reported configurations

| **Configuration** | **First round SD; range; 95% CI** | **Two rounds SD; range; 95% CI** | **Recovery 95% CI** |
| --- | --- | --- | --- |
| Base | 0.58; 0-1; 0-1.25 | 1.29; 3-6; 2.25-6.75 | 7.8-24.8% |
| Luna | 1.89; 5-9; 3.25-9.5 | 1.50; 15-18; 12.75-20.5 | 36.8-70% |
| SFT v5 | 1.63; 4-8; 3-9.25 | 1.71; 8-12; 7-13.75 | 11.9-31.6% |
| Synth v1 | 0.58; 9-10; 5.75-13.5 | 0.82; 11-13; 8.25-15.75 | 7.4-24.4% |
| Midband RL v1 | 2.06; 6-11; 4.75-12 | 0.82; 12-14; 9-17 | 14.5-41.4% |
| Midband RL v2 | 1.29; 9-12; 7-14 | 2.08; 12-17; 10.5-18.5 | 12.9-41.9% |
| Phone Number | 0.50; 11-12; 7.5-15 | 0.50; 15-16; 11-19.25 | 13.1-44.2% |
| Generalized C++\* | 1.50; 10-13; 8-15.5 | 1.41; 14-17; 11.75-20 | 14.5-48.9% |
...
## B.2 Inspect the branch, then follow the result-specific artifacts
...
artifacts used in each historical experiment. [[1, 6]](#references)
...
1. [Project release: source and Fixed26 results](https://github.com/tokenbender/browser-is-all-you-need-upstream/tree/f4b77ab623c5fed9b9503d49d77a86fa1f4299ee). client/21-sept-release, September 2026. README, result tables, experiment contracts, and artifact entry points.
2. [Phone Number curriculum and kernel reward](https://github.com/tokenbender/browser-is-all-you-need-upstream/blob/f4b77ab623c5fed9b9503d49d77a86fa1f4299ee/Reward_GRPO/phone_number_grpo.py). Eight task contexts, twelve executable checks, task generation, receipt handling, and reward composition.
3. [Whole-file editing, parsing, execution, and shared reward](https://github.com/tokenbender/browser-is-all-you-need-upstream/tree/f4b77ab623c5fed9b9503d49d77a86fa1f4299ee/src/glm47_posttraining/aider_polyglot). Released task schema, parser, harness, and shared reward implementation.
4. [Generalized C++ and topic-level verification](https://github.com/tokenbender/browser-is-all-you-need-upstream/tree/f4b77ab623c5fed9b9503d49d77a86fa1f4299ee/Reward_GRPO). `generalized_cpp_grpo.py`, `generalized_cpp_topic_grpo.py`, `global_cpp_verifier_runner.py`, and `topic_coverage/`.
5. [GLM/Miles training and rollout integration](https://github.com/tokenbender/browser-is-all-you-need-upstream/tree/f4b77ab623c5fed9b9503d49d77a86fa1f4299ee/src/glm47_posttraining/integrations). Adapter loading and synchronization, infrastructure score handling, and training integration.
6. [Released experiment evidence](https://github.com/tokenbender/browser-is-all-you-need-upstream/tree/f4b77ab623c5fed9b9503d49d77a86fa1f4299ee/results). Result manifests, `per_task_success.csv`, `statistics.json`, run receipts, reconstruction checks, and launch configurations.
7. [Shao et al. DeepSeekMath: Pushing the Limits of Mathematical Reasoning in Open Language Models](https://arxiv.org/abs/2402.03300). 2024. Introduces Group Relative Policy Optimization.
8. [Wang et al. LETI: Learning to Generate from Textual Interactions](https://aclanthology.org/2024.findings-naacl.16/). Findings of NAACL, 2024. Execution-derived textual feedback for code generation.
9. [Yang et al. SWE-smith: Scaling Data for Software Engineering Agents](https://arxiv.org/abs/2504.21798). 2025. Synthetic executable task construction for software-engineering agents.
10. [Yang et al. SWE-agent: Agent-Computer Interfaces Enable Automated Software Engineering](https://arxiv.org/abs/2405.15793). 2024. Agent interface design and its effects on software-engineering performance.
11. [Kim et al. FrogNano: Training a 4B Coding Agent via Online Task Synthesis](https://arxiv.org/abs/2609.07925v4). 2026, version 4. Harness design, policy-adaptive task synthesis, and iterative reinforcement learning.
12. [Hu et al. LoRA: Low-Rank Adaptation of Large Language Models](https://arxiv.org/abs/2106.09685). 2021. Low-rank trainable updates to frozen pretrained model weights.
13. [Bank-account curriculum construction and verification](https://github.com/tokenbender/browser-is-all-you-need-upstream/blob/f4b77ab623c5fed9b9503d49d77a86fa1f4299ee/src/glm47_posttraining/aider_polyglot/bank_account_curriculum.py). Reference implementations, build and state mutations, distinct controls, and packaged practice contexts.
